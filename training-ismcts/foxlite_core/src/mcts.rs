...
pub const TEMP_OPENING: f64 = 1.0; // sampling temperature at each round's first trick
pub const TEMP_FLOOR: f64 = 0.25; // sampling temperature at each round's last trick

/// Self-play move-selection temperature, annealed *per round*: it restarts at
/// `TEMP_OPENING` on each round's first trick (`trick_num == 1`) and ramps linearly
/// to `TEMP_FLOOR` across the round's `TRICKS_PER_ROUND` tricks. With per-round
/// value targets each round is its own reward episode, so the schedule resets with
/// it rather than annealing once across the whole match.
pub fn temperature(trick_num: u32) -> f64 {
    let frac = ((trick_num.max(1) - 1) as f64 / (TRICKS_PER_ROUND - 1) as f64).clamp(0.0, 1.0);
